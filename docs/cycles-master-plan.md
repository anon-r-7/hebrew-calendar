...
- Cycles tab: grouped period menu (fractions of 8190 small to large, the rung, its notable multiples k = 4 / 16 / 44 / 56 / 90 / 91, then Revelation / Enochian / jubilee / sabbatical years), anchor, tolerance, and a remembered three-way view: **Event groupings** (events sharing the same residue from the anchor, i.e. whole periods apart; a plain-language note explains residues; at ±N a group is a reference member plus the members that miss it by exactly that bucket), **Ladder** (the anchor stepped along the period: for 8190 every multiple of 44, 56, 90 and 91; for a multiple every step; for a fraction only steps with an event within two weeks; columns k, Revelation, Enochian, Hebrew, Gregorian, holidays on the day and events within two weeks; not offered for the year periods), and **From anchor** (every event read from the anchor, best first).
...
**Generator.** `yarn pairs:generate [--min-score 5.5] [--tol 2] [--favorite 8] [--reset] [--dry-run]` creates every pair the engine scores at or above the threshold (earlier → later), skipping existing pairs; `--reset` deletes all pairs first. On the 0–10 scale 5.5 means a ⅕-rung fraction or better, 8 a whole rung.
